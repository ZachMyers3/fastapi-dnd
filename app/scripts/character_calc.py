...
def calc_spells_available(character, mongo):
    ...
    for cclass in character["class"]:
        ...
        class_level = mongo.db.levels.find_one(
            {"level": cclass["level"], "class.name": cclass["name"]}
        )
        ...
        class_spells = list(mongo.db.spells.find({"classes.class": cclass["name"]}))
        for spell in class_spells:
            ...
            spell_list.append(
                {"name": spell["name"], "level": spell["level"], "id": spell["id"]}
            )
